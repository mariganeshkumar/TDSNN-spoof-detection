...
parfor i=1:length(genuineFileList);
    ...
    [statG,deltaG,double_deltaG]=extract_lfbe(x,fs,windowLength,nFFT,filterCount) ;
    ...
    header=size(var');
%    filePath1 = fullfile('..','dummy.lfbe');
    filePath1 = fullfile(pathToGenuineTrainFeatFiles,'train/bonafide',strcat(fileName{1},'.lfbe'));
...
parfor i=1:length(replayFileList)
    ...
    [x,fs] = audioread(filePath);
    [statR,deltaR,double_deltaR]=extract_lfbe(x,fs,windowLength,nFFT,filterCount) ;
    if ( string(featureType{1})  == 'S' )
        %disp('only S');
    ...
    header=size(var');
    filePath1 = fullfile(pathToReplayTrainFeatFiles,'train/spoofed',strcat(fileName{1},'.lfbe'));
    dlmwrite(filePath1,header,'delimiter',' ','precision','%d');
    dlmwrite(filePath1,var,'-append','newline','unix','delimiter',' ','precision','%.6f');
end
disp('Done!');
...
parfor i=1:length(devFileList)
    ...
    [x,fs] = audioread(filePath);
    [statD,deltaD,double_deltaD]=extract_lfbe(x,fs,windowLength,nFFT,filterCount);
    if ( string(featureType{1})  == 'S' )
        %disp('only S');
	...
	if (spfFlag==0)
		filePath1 = fullfile(pathToDevFeatures,'spoofed',strcat(fileName{1},'.lfbe'))
	else
		filePath1 = fullfile(pathToDevFeatures,'bonafide',strcat(fileName{1},'.lfbe'))
	end
	dlmwrite(filePath1,header,'delimiter',' ','precision','%d');
	dlmwrite(filePath1,var,'-append','newline','unix','delimiter',' ','precision','%.6f');
end
