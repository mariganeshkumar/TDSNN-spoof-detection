...
parfor i=1:length(genuineFileList);
    ...
    [statG,deltaG,double_deltaG]=extract_imfcc(x,fs,windowLength,nFFT,filterCount,cepstrumCount) ;
    ...
    header=size(var');
%    filePath1 = fullfile('..','dummy.imfcc');
    filePath1 = fullfile(pathToGenuineTrainFeatFiles,'train/bonafide',strcat(fileName{1},'.imfcc'));
...
parfor i=1:length(replayFileList)
    ...
    [x,fs] = audioread(filePath);
    [statR,deltaR,double_deltaR]=extract_imfcc(x,fs,windowLength,nFFT,filterCount,cepstrumCount) ;
    if ( string(featureType{1})  == 'S' )
        %disp('only S');
    ...
    header=size(var');
    filePath1 = fullfile(pathToReplayTrainFeatFiles,'train/spoofed',strcat(fileName{1},'.imfcc'));
    dlmwrite(filePath1,header,'delimiter',' ','precision','%d');
    dlmwrite(filePath1,var,'-append','newline','unix','delimiter',' ','precision','%.6f');
end
disp('Done!');
...
parfor i=1:length(devFileList)
    ...
    [x,fs] = audioread(filePath);
    [statD,deltaD,double_deltaD]=extract_imfcc(x,fs,windowLength,nFFT,filterCount,cepstrumCount);
    if ( string(featureType{1})  == 'S' )
        %disp('only S');
	...
	if (spfFlag==0)
		filePath1 = fullfile(pathToDevFeatures,'spoofed',strcat(fileName{1},'.imfcc'))
	else
		filePath1 = fullfile(pathToDevFeatures,'bonafide',strcat(fileName{1},'.imfcc'))
	end
	dlmwrite(filePath1,header,'delimiter',' ','precision','%d');
	dlmwrite(filePath1,var,'-append','newline','unix','delimiter',' ','precision','%.6f');
end
