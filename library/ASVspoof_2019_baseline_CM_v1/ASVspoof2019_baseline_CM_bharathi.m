...
feature_type = 'LFBE'; % LFCC or CQCC or IMFCC or LFBE
...
parfor i=1:length(bonafideIdx)
    ...
    elseif strcmp(feature_type,'CQCC')
        genuineFeatureCell{i} = cqcc(x, fs, 96, fs/2, fs/2^10, 16, 40, 'ZsdD');
...
parfor i=1:length(spoofIdx)
    ...
    elseif strcmp(feature_type,'CQCC')
        spoofFeatureCell{i} = cqcc(x, fs, 96, fs/2, fs/2^10, 16, 40, 'ZsdD');
    elseif strcmp(feature_type,'IMFCC')
          [stat,delta,double_delta] = extract_imfcc_bharathi(x,fs,20,512,30,20);
           spoofFeatureCell{i} = [stat delta double_delta ]';
    end
...
[genuineGMM.m, genuineGMM.s, genuineGMM.w] = vl_gmm([genuineFeatureCell{:}], 1024, 'verbose', 'MaxNumIterations',10);
disp('Done!');

% train GMM for SPOOF data
disp('Training GMM for SPOOF...');
[spoofGMM.m, spoofGMM.s, spoofGMM.w] = vl_gmm([spoofFeatureCell{:}], 1024, 'verbose', 'MaxNumIterations',10);
disp('Done!');
...
parfor i=1:length(filelist)
    ...
    elseif strcmp(feature_type,'CQCC')
        x_fea = cqcc(x, fs, 96, fs/2, fs/2^10, 16, 40, 'ZsdD');
    elseif strcmp(feature_type,'IMFCC')
          [stat,delta,double_delta] = extract_imfcc_bharathi(x,fs,20,512,30,20);
           x_fea = [stat delta double_delta ]';
    end
