...
feature_type = 'MFCC'; % LFCC or CQCC or IMFCC or LFBE
...
pathToASVspoof2019Data = '../data/ASV_spoof_2019/';
...
parfor i=1:length(bonafideIdx)
    ...
    if strcmp(feature_type,'LFBE')
        [stat,delta,double_delta] = extract_lfbe(x,fs,20,512,60);
        genuineFeatureCell{i} = [stat delta double_delta]';
    elseif strcmp(feature_type,'CQCC')
        genuineFeatureCell{i} = cqcc(x, fs, 96, fs/2, fs/2^10, 16, 13, 'ZsdD');
    elseif strcmp(feature_type,'IMFCC')
        [stat,delta,double_delta] = extract_imfcc_bharathi(x,fs,20,512,30,20);
        genuineFeatureCell{i} = [stat delta double_delta ]';
    elseif strcmp(feature_type,'LFCC')
        [stat,delta,double_delta] = extract_lfcc(x,fs,20,512,40,20);
        genuineFeatureCell{i} = [stat delta double_delta ]';
    elseif strcmp(feature_type,'MFCC')
        [stat,delta,double_delta] = mfcc(x,fs,'WindowLength',round(fs*0.25),'OverlapLength',round(fs*0.10),'NumCoeffs',20);
        genuineFeatureCell{i} = [stat delta double_delta ]';
    end
end
disp('Done!');
...
parfor i=1:length(spoofIdx)
    ...
    elseif strcmp(feature_type,'CQCC')
        spoofFeatureCell{i} = cqcc(x, fs, 96, fs/2, fs/2^10, 16, 13, 'ZsdD');
    elseif strcmp(feature_type,'IMFCC')
          [stat,delta,double_delta] = extract_imfcc_bharathi(x,fs,20,512,30,20);
           spoofFeatureCell{i} = [stat delta double_delta ]';
    elseif strcmp(feature_type,'LFCC')
	  disp('Extracting LFCC feats')
          [stat,delta,double_delta] = extract_lfcc(x,fs,20,512,40,20);
          spoofFeatureCell{i} = [stat delta double_delta ]';
    elseif strcmp(feature_type,'MFCC')
          [stat,delta,double_delta] = mfcc(x,fs,'WindowLength',round(fs*0.25),'OverlapLength',round(fs*0.10),'NumCoeffs',20);
...
devFeatureCell = cell(size(filelist));
parfor i=1:length(filelist)
	...
	devFeatureCell{i} = x_fea;
    elseif strcmp(feature_type,'CQCC')
        x_fea = cqcc(x, fs, 96, fs/2, fs/2^10, 16, 13, 'ZsdD');
	devFeatureCell{i} = x_fea;
    elseif strcmp(feature_type,'IMFCC')
        [stat,delta,double_delta] = extract_imfcc_bharathi(x,fs,20,512,30,20);
        x_fea = [stat delta double_delta ]';
	...
	disp('Not LFBE or CQCC or LFCC or IMFCC')
    end
    ...
    llk_genuine = mean(compute_llk(devFeatureCell{i},genuineGMM.m,genuineGMM.s,genuineGMM.w));
    llk_spoof = mean(compute_llk(devFeatureCell{i},spoofGMM.m,spoofGMM.s,spoofGMM.w));
